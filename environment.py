...
def _safe_episode_reward(value: float) -> float:
    """
    Clamp episode_reward to be strictly within (-1.0, 1.0).
    The Reward model allows ge=-1.0, le=1.0 but the validator
    requires strictly between 0 and 1 at the task-score level.
    We keep a small epsilon away from both boundaries.
    """
...
class EmailTriageEnv:
    ...
    def _apply_multi_turn_consequences(self, email: Email, base_reward: float, action: Action) -> float:
        ...
        if email.customer_lifetime_value > 0.9 or email.scenario_context == "vip_opportunity":
            if base_reward > 0.8:
                final_reward = base_reward * 1.5
            elif base_reward < 0.3:
                final_reward = base_reward * 0.5
        ...
        if "outage" in email.subject.lower() or (email.scenario_context and "outage" in email.scenario_context):
            if base_reward > 0.7:
                final_reward = base_reward * 1.4
            else:
                final_reward = base_reward * 0.7

        # Security/Compliance
        if any(kw in email.subject.lower() for kw in ["security", "breach", "vulnerability"]):
            if base_reward > 0.8:
                final_reward = base_reward * 1.3
            else:
                final_reward = base_reward * 0.4
                ...
                if "cascade" in history_item.get("email_id", ""):
                    ...
                    if prev_cascade_reward < 0.5:
                        if base_reward > 0.8:
                            ...
                            final_reward = base_reward * 0.6

        # Deadline scenarios
        if email.scenario_context and "deadline" in email.scenario_context.lower():
            if base_reward > 0.7:
                final_reward = base_reward * 1.2
            else:
                final_reward = base_reward * 0.8

        # Enterprise/Strategic opportunities
        if email.scenario_context and any(kw in email.scenario_context.lower() for kw in ["enterprise", "partnership"]):
            if base_reward > 0.8:
                final_reward = base_reward * 1.4
            else:
                final_reward = base_reward * 0.5
    ...
    def _get_grading(self, email: Email, action: Action) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            if self.task_id == "email_priority_classification":
                ...
                return normalize_score(0.0), {"error": "Unknown task"}
        except Exception as e:
        ...
        try:
            if self.episode_done:
                return self._get_observation(), Reward(
                    episode_reward=0.01,   # never exactly 0.0
                    cumulative_reward=max(-9.99, min(9.99, self.cumulative_reward)),
                    is_done=True,
            ...
            try:
                ...
                base_reward = normalize_score(0.5)   # safe neutral via normalize_score
                grading_details = {"error": str(e), "grading_method": "fallback"}

            # Apply multi-turn consequences
            try:
            ...
            # Clamp episode_reward strictly to (-0.99, 0.99) — never hits -1.0 or 1.0
            step_reward = _safe_episode_reward(raw_step_reward)

            self.step_count += 1
            self.episode_reward = step_reward
            self.cumulative_reward += step_reward

            # Clamp cumulative reward to valid model range, keeping away from hard bounds
            self.cumulative_reward = max(-9.99, min(9.99, self.cumulative_reward))

            # Cache action for multi-turn reference
            try:
